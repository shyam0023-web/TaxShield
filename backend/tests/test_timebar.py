...
import sys
sys.path.insert(0, '..')
from app.watchdog.timebar import calculate_timebar

def test_section_73_time_barred():
    """Section 73: FY 2018-19 should be time-barred by Jan 2024"""
    result = calculate_timebar(fy="2018-19", section=73, notice_date=date(2024, 1, 15))
...
def test_section_74_not_time_barred():
    ...
    result = calculate_timebar(fy="2020-21", section=74, notice_date=date(2024, 1, 15))
    assert result.is_time_barred == False

def test_section_73_valid():
    """Section 73: Recent FY should be valid"""
    result = calculate_timebar(fy="2022-23", section=73, notice_date=date(2024, 1, 15))
    assert result.is_time_barred == False
